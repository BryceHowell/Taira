...
class SpriteParser {
	...
	static Vector<Scene> fileRead(String filename, BufferedImage gateSprite) {   // image library should be passed in here
			...
			while (in.ready()) {
				...
				while (m.find()) {
					...
					if (t.name().equalsIgnoreCase("brush") && t.start()) { // needs to check for named brushes. will need a register for that 
						...
						if (t.hasAttribute("image")) { b.imagename=t.getValue("image"); b.gfx=Brush.DECAL; b.decal=gateSprite; }
